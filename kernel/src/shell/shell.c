...
static void shell_execute(const char *cmd) {
    ...
    if (memcmp(cmd, "help", 4) == 0 &&
        ...
        console_printf(
            "[SYS] Uptime Ticks: %u (%u seconds)\n",
            ticks,
            ticks / 100
        );

        console_printf(
            "[SYS] Bootloader: Limine Protocol v6\n\n"
        );
        ...
        console_printf(
            "  Total Heap Size : %u KB (%u bytes)\n",
            (uint32_t)(total / 1024),
            (uint32_t)total
        );

        console_printf(
            "  Used Memory     : %u KB (%u bytes)\n",
            (uint32_t)(used / 1024),
            (uint32_t)used
        );

        console_printf(
            "  Free Memory     : %u KB (%u bytes)\n\n",
            (uint32_t)(free_b / 1024),
            (uint32_t)free_b
        );

    } else if (memcmp(cmd, "ps", 2) == 0 &&
               (cmd[2] == '\0' || cmd[2] == ' ')) {
    ...
    } else if (memcmp(cmd, "sched", 5) == 0 &&
               (cmd[5] == '\0' || cmd[5] == ' ')) {

        scheduler_start_demo();

    } else if (memcmp(cmd, "ipc", 3) == 0 &&
               (cmd[3] == '\0' || cmd[3] == ' ')) {
...
void shell_init(void) {
    ...
    /*
     * Cursor temporarily disabled while
...
void shell_update(void) {
    ...
    /*
     * Auto-Demo trigger after approximately 2.5 seconds.
     */
    if (!auto_demo_done &&
        (timer_get_ticks() - start_ticks > 250)) {

        auto_demo_done = true;

        console_write(
            "\n\n[AUTO-DEMO] "
            "Demonstrating LedgerOS Core OS Concepts...\n"
        );
        ...
        console_write(
            "[AUTO-DEMO COMPLETE] "
            "LedgerOS is ready for interactive input.\n\n"
        );
